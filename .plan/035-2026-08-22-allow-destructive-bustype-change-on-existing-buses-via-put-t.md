...
- **Frontend follow-up (in scope, not optional):** `frontend/src/modals/BusModal.tsx`'s warning banner currently says the change "ימחק את כל שיוכי הנוסעים הקיימים" ("will delete ALL existing passenger assignments") — this must be corrected to describe the accurate, narrower risk now that reseed is by-position (e.g. "מושבים שלא קיימים בדגם החדש יאבדו את שיוך הנוסע; מושבים קיימים בדגם החדש ישמרו את השיוך שלהם"). The confirm-checkbox gate itself (`confirmReset`) stays — changing bus type on an occupied bus should still require explicit confirmation, just with accurate wording. `frontend/src/services/bus.service.ts`'s comment ("DESTRUCTIVE ON UPDATE ... discarding any existing seat assignments") needs the same correction.
