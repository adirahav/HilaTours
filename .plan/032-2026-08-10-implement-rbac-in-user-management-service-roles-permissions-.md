...
## Scope
- In scope:
  - `backend/user-management-service`: add `GET /role` and `GET /permission` read-only endpoints per the API contract (`docs/api-contract/api-contract.user-management-service.yaml` paths `/role`, `/permission`), backed by the existing `Role`/`Permission` Mongoose models.
...
- Explicitly out of scope: adding a role-assignment/promotion endpoint (`PUT /admin/{id}/role` or similar) — `database-rules.md` and the API contract both list this as an open question / manual-only action today, not part of this task.

## Assumptions
- The task's phrasing "`GET /role` and `GET /permission`" refers loosely to the same read-only lookups the API contract defines as `GET /role` and `GET /permission` (plural, under a "Roles & Permissions" tag) — this plan follows the actual contract path names, not the singular forms in the backlog title (flagged in Open Questions).
...
- Should `tour-service`'s new role check be a full permission-key check (e.g. `POST /tour` requires `tour:insert`) or the coarser `roles.includes('admin')` check already used by `user-management-service`'s own `requireAdmin`?
  ...
  - *HUMAN ANSWER:* full permission-key check (e.g. `POST /tour` requires `tour:insert`)
- The backlog task names the endpoints `GET /role` / `GET /permission` (singular); the API contract defines `GET /role` / `GET /permission` (plural). Which should the implementation use?
  - Recommended: follow the API contract (`/role`, `/permission`) since it's the more detailed, versioned source of truth (`v1.2.0`) and is what `frontend` or any future admin-management UI would be built against.
  - *HUMAN ANSWER:* /role

## Steps
1. `user-management-service` agent:
   - Add `backend/user-management-service/api/roles-permissions/roles-permissions.controller.ts` (or similar) exposing `GET /role` (returns `Role.find()` sorted by `name`, `toJSON` already strips `_id`/`__v`/renames `uuid`→`id`) and `GET /permission` (`Permission.find()` sorted by `category`,`key`), both behind `requireAuth` per the contract's `security: [bearerAuth]`.
...
4. `security` agent reviews: that `GET /role`/`GET /permission` don't leak anything beyond the documented `Role`/`Permission` shape, and that the tightened `tour-service` `requireAdmin` doesn't introduce a bypass (e.g. missing-roles-array must fail closed, not open).
...
2. `user-management-service` `GET /role` / `GET /permission` endpoints.
